...
void setup() {
  // put your setup code here, to run once:
    ...
    if(digitalRead(DB_LED_AND_UPDATE_PIN)){
        ...
        // no idea where updae_ids is defined
        //set_connected(0); // init as not connected
        if(true){ // true = force update, update_ids will initiate the connect
          
        
          //  set_connected(1);
        } else {
            // set_connected(true,0); // force LED update for not connected
            EEPROM.read(address);
        }
        // ############ MACS MODUS ############ // 
...
void loop() {
    ...
    if((digitalRead(TAG_IN_RANGE_INPUT)==0 || Serial.available()) && currentTag!=-1){ 
